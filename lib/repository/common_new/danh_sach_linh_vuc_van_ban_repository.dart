import 'package:eportal/api/adapter/base/base_adapter_api.dart';
import 'package:eportal/model/api/request/common_new/danh_sach_linh_vuc_van_ban_request.dart';
import 'package:eportal/model/api/response/common_new/danh_sach_linh_vuc_van_ban_response.dart';

//
// Created by BlackRose on 21/11/2023.
// Copyright (c) 2023 Hilo All rights reserved.
//
class DanhSachLinhVucVanBanRepository {
  Future<DanhSachLinhVucVanBanResponse> getDanhSachLinhVucVanBan(
      DanhSachLinhVucVanBanRequest request) {
    return BaseAdapterApi()
        .callApiAsync(request)
        .then((value) => DanhSachLinhVucVanBanResponse.fromJson(value));
  }
}
