...
class DocumentListBloc extends Bloc<BaseEvent, BaseState> {
  DocumentListBloc() : super(BaseInitial()) {
    final DocumentListRepository apiRepository = DocumentListRepository();

    on<DocumentListEvent>((event, emit) async {
      try {
        emit(BaseLoading());
        final response = await apiRepository.getDocumentList(event.request);
